...
まずは、基本的なクラス構造と DOM 要素の取得部分を作成しましょう。
...
- `document.getElementById("todoInput")`: `id="todoInput"`を持つ HTML 要素を取得するための命令です。
...
- `addEventListener`: click や keypress といったイベント（ユーザーの操作）を監視し、指定された処理を実行します。
...
- onchange や onclick 属性に直接 JavaScript のコードを記述しています。todoApp というグローバル変数（DOMContentLoaded 内で定義）を通じて、クラスのメソッドを呼び出しています。
...
- `classList.toggle("active", 条件)`は、条件が true なら active クラスを追加し、false なら削除する便利なメソッドです。これにより、押されたボタンだけがハイライトされます。
...
script.js の TodoApp クラス内にメソッドを追加:
...
- confirm()は、ユーザーに「OK」「キャンセル」を求めるダイアログを表示し、OK が押されたら true を返します。誤操作を防ぐために重要な機能です。
